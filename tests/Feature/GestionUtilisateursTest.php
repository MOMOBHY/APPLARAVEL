...
class GestionUtilisateursTest extends TestCase
{
    ...
    {
        $users = collect(
            $this->actingAs($this->admin, 'sanctum')->getJson('/api/users')->json('users'),
        )->keyBy('matricule');
    ...
    public function test_compte_suspendu_ne_peut_plus_se_connecter_et_perd_ses_jetons(): void
    {
        ...
        $this->actingAs($this->admin, 'sanctum')
            ->postJson('/api/users/update', ['matricule' => 'AGT001', 'actif' => false])
            ->assertOk();
        ...
        $statut = collect(
            $this->actingAs($this->admin, 'sanctum')->getJson('/api/users')->json('users'),
        )->firstWhere('matricule', 'AGT001')['statut'];
        $this->assertSame('SUSPENDU', $statut);

        $this->actingAs($this->admin, 'sanctum')
            ->postJson('/api/users/update', ['matricule' => 'AGT001', 'actif' => true])
            ->assertOk();
        $this->postJson('/api/login', [
            'matricule' => 'AGT001',
            'password' => 'test123',
    ...
    {
        $this->actingAs($this->admin, 'sanctum')
            ->postJson('/api/users/update', ['matricule' => 'ADM001', 'actif' => false])
            ->assertUnprocessable();
        $this->actingAs($this->admin, 'sanctum')
            ->postJson('/api/users/update', ['matricule' => 'ADM001', 'role' => 'ROLE_AGENT'])
            ->assertUnprocessable();
