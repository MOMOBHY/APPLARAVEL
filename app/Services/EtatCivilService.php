 ...
 * des permissions :
 * AGENT → SERVICE ADMINISTRATIF (contrôle) → DRH (validation) → AGENT.
 * Service administratif = GESTIONNAIRE RH (rôle existant, pas de GRH notes).
 */
class EtatCivilService
{
    public const EN_ATTENTE_SERVICE = 'EN_ATTENTE_SERVICE_GESTION_ADMINISTRATIVE';
    ...
    public static function declarer(string $type, Agent $agent, array $data, ?string $piecePath): DeclarationNaissance|DeclarationDeces
    {
        ...
        $communs = [
            ...
            'agent_id' => $agent->id,
            'statut' => $soumettre ? self::EN_ATTENTE_SERVICE : self::BROUILLON,
        ];
        ...
        self::tracer($type, $declaration, $agent, 'ROLE_AGENT',
            ...
            $soumettre ? 'Déclaration transmise au service administratif.' : 'Brouillon enregistré.');

        if ($soumettre) {
            self::avertirService($type, $declaration, $agent);
        }
    ...
    public static function soumettreBrouillon(string $type, DeclarationNaissance|DeclarationDeces $declaration, Agent $agent): DeclarationNaissance|DeclarationDeces
    {
        ...
        $declaration->update(['statut' => self::EN_ATTENTE_SERVICE]);
        self::tracer($type, $declaration, $agent, 'ROLE_AGENT', 'SOUMISSION', $ancien, $declaration->statut,
            'Brouillon soumis au service administratif.');
        self::avertirService($type, $declaration, $agent);
    ...
    // ---------------------------------------------------------------
    // SERVICE ADMINISTRATIF : contrôler (conforme / retourner)
    // ---------------------------------------------------------------
    /**
    ...
    {
        if ($declaration->statut !== self::EN_ATTENTE_SERVICE) {
            abort(422, 'Contrôle impossible à ce stade.');
        }
        ...
        if ($decision === 'retourner') {
            ...
            self::tracer($type, $declaration, $controleur, 'ROLE_SERVICE_ADMINISTRATIF', 'RETOUR_CORRECTION', $ancien, $declaration->statut, $motif);
            self::notifier($declaration->agent_id, 'Dossier incomplet à corriger',
                "Votre déclaration {$declaration->code_dossier} est retournée : {$motif}", 'RETOUR', $declaration->code_dossier);

            return $declaration->refresh();
        }

        $declaration->update(['statut' => self::EN_ATTENTE_RH, 'motif_retour' => null]);
        self::tracer($type, $declaration, $controleur, 'ROLE_SERVICE_ADMINISTRATIF', 'CONTROLE_CONFORME', $ancien, $declaration->statut,
            'Dossier conforme, transmis à la DRH.');

        $drh = PermissionWorkflowService::drh();
        if ($drh) {
            self::notifier($drh->id, 'Acte à valider',
                "Déclaration {$declaration->code_dossier} contrôlée conforme, décision DRH requise.", 'ATTENTE_DRH', $declaration->code_dossier);
        }
    ...
    public static function corriger(string $type, DeclarationNaissance|DeclarationDeces $declaration, Agent $agent, array $data, ?string $piecePath): DeclarationNaissance|DeclarationDeces
    {
        ...
        $ancien = $declaration->statut;
        $maj = ['statut' => self::EN_ATTENTE_SERVICE, 'motif_retour' => null];
        ...
        self::tracer($type, $declaration, $agent, 'ROLE_AGENT', 'CORRECTION', $ancien, $declaration->statut,
            'Dossier corrigé et renvoyé au service administratif.');
        self::avertirService($type, $declaration, $agent);
    ...
    protected static function avertirService(string $type, DeclarationNaissance|DeclarationDeces $declaration, Agent $agent): void
    {
        $libelle = $type === 'NAISSANCE' ? 'naissance' : 'décès';
        $gestionnaire = PermissionWorkflowService::gestionnairePour($agent);
        if ($gestionnaire) {
            self::notifier($gestionnaire->id, "Déclaration de {$libelle} à contrôler",
        ...
        self::notifier($agent->id, 'Déclaration transmise',
            "Votre déclaration {$declaration->code_dossier} est transmise au service administratif.", 'INFO', $declaration->code_dossier);
    }
