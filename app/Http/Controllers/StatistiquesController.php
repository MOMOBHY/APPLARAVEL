...
{
    private const NATURES = ['permission' => 'Permissions', 'naissance' => 'Naissances', 'deces' => 'Décès'];

    public function index()
    {
        $dossiers = $this->dossiers();

        $parStatut = collect(['validee', 'a_corriger', 'en_attente', 'rejetee'])
            ->mapWithKeys(fn (string $statut) => [$statut => $dossiers->where('statut', $statut)->count()]);
        ...
        return response()->json([
            ...
            'indicateurs' => [
                ...
                'taux_validation' => $tranches ? round($parStatut['validee'] * 100 / $tranches) : null,
                'delai_moyen_jours' => $this->delaiMoyen($dossiers),
            ],
            'par_mois' => $this->parMois($dossiers),
            'par_statut' => $parStatut,
            'par_structure' => $dossiers->groupBy('structure')
                ->map(fn (Collection $groupe, string $structure) => ['structure' => $structure, 'total' => $groupe->count()])
                ->sortByDesc('total')->values(),
            'delai_par_nature' => collect(self::NATURES)->map(fn (string $libelle, string $nature) => [
                'nature' => $libelle,
                'delai_moyen_jours' => $this->delaiMoyen($dossiers->where('nature', $nature)),
                'dossiers_clos' => $dossiers->where('nature', $nature)->whereNotNull('cloture')->count(),
            ])->values(),
        ]);
    }
    ...
    {
        $permissions = DemandePermission::with('agent.structure')->get()->map(fn (DemandePermission $d) => [
            'nature' => 'permission',
            'depot' => $d->created_at,
            'statut' => $this->categorie($d->statut, $d->motif_rejet),
            // Décision DRH, ou refus de visa, ou rejet à la vérification.
            'cloture' => in_array($d->statut, [DemandePermission::VALIDEE, DemandePermission::REJETEE], true)
                ? ($d->date_decision ?? $d->date_visa ?? $d->date_verif_rh)
                : null,
            'structure' => $d->agent?->structure?->nom ?? 'Sans structure',
        ]);

        $declarations = fn (string $nature, Collection $lignes) => $lignes->map(fn ($d) => [
            'nature' => $nature,
            'depot' => $d->created_at,
            'statut' => $this->categorie($d->statut, $d->motif_rejet),
        ...
        return $permissions
            ->concat($declarations('naissance', DeclarationNaissance::with('agent.structure')->get()))
    ...
    {
        $delais = $dossiers->whereNotNull('cloture')
    ...
    private function parMois(Collection $dossiers): Collection
    {
        ...
        return collect(range(0, 11))->map(function (int $i) use ($debut, $dossiers) {
            ...
            $duMois = $dossiers->filter(fn (array $d) => $d['depot']?->format('Y-m') === $mois->format('Y-m'));

            return ['mois' => $mois->format('Y-m'), 'total' => $duMois->count()]
                + collect(self::NATURES)->map(fn ($libelle, string $nature) => $duMois->where('nature', $nature)->count())->all();
        });
    }
}
