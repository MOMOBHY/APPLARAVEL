...
class PieceController extends Controller
{
    ...
    {
        abort_unless(
            PieceService::autorise($request->user(), $piece),
            403,
            'Accès non autorisé à cette pièce.',
        );
        JournalAudit::noter(
            JournalAudit::PIECE,
            'PIECE_CONSULTEE',
            $request->user(),
            "Consultation du justificatif {$piece->nom_fichier}",
            $piece->reference_dossier,
        );
        abort_unless(
            Storage::disk('local')->exists($piece->chemin_stockage),
            404,
            'Fichier introuvable.',
        );
